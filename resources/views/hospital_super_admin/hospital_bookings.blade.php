...
@section('title', $hospital->hospital_name . ' - Patient Bookings')
...
<div class="page-header" style="display: flex; align-items: flex-start; justify-content: space-between; gap: 24px; margin-bottom: 32px;">
    <div>
        <div style="display: flex; align-items: center; gap: 18px;">
            <h1 style="font-size:2.1rem;font-weight:900;color:#1e3a8a;line-height:1.1;margin:0;">
                {{ $hospital->hospital_name }} - Patient Bookings
            </h1>
    ...
    <a href="{{ route('hospital_super_admin.bookings') }}"
        style="
                display:inline-flex;
                align-items:center;
                gap:8px;
                ...
                font-weight:700;
                box-shadow:0 4px 12px rgba(19,99,198,0.08);
                transition:all .2s ease;
        "
        onmouseover="this.style.background='#1363C6';this.style.color='#fff'"
        onmouseout="this.style.background='#fff';this.style.color='#1363C6'">
...
<div class="filter-bar-modern">
    <form method="GET" style="display: flex; align-items: center; gap: 18px; flex-wrap: wrap; width:100%;">
        ...
        <div class="filter-actions">
            ...
            <a href="{{ route('hospital_super_admin.bookings.hospital', $hospital->id) }}" class="btn-saas secondary">
                Reset
            </a>
        </div>
    </form>
...
onclick="hsaUpdateStatus({{ $booking->id }}, 'accepted')">
✓ Accept
</button>

<button class="saas-dropdown-btn reject"
onclick="hsaUpdateStatus({{ $booking->id }}, 'rejected')">
✕ Reject
</button>

<button class="saas-dropdown-btn resched"
onclick="hsaOpenRescheduleModal({{ $booking->id }}, '{{ $booking->patient_name }}','{{ $booking->booking_date }}','{{ $booking->start_time }}')">
Reschedule
</button>

<button class="saas-dropdown-btn resched"
onclick="hsaOpenAssignModal({{ $booking->id }}, '{{ $booking->patient_name }}')">
Assign
</button>
...
onclick="hsaUpdateStatus({{ $booking->id }}, 'no_show')">
No Show
</button>
...
onclick="hsaUpdateStatus({{ $booking->id }}, 'completed')">
Complete
</button>
...
<button class="saas-three-dot-btn" onclick="toggleActions(this, event)">
    &#x22EF;
</button>
...
<div id="rescheduleModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <button class="close-btn" onclick="hsaCloseRescheduleModal()">&times;</button>
        </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn-modal btn-cancel" onclick="hsaCloseRescheduleModal()">Cancel</button>
            <button type="button" class="btn-modal btn-submit" onclick="hsaSubmitReschedule()">Confirm Reschedule</button>
        </div>
    </div>
</div>
...
<div id="assignModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <button class="close-btn" onclick="hsaCloseAssignModal()">&times;</button>
        </div>
        <div class="modal-body">
            <div class="patient-info-box">
                <h4>Patient</h4>
        ...
        <div class="modal-footer">
            <button class="btn-modal btn-cancel" onclick="hsaCloseAssignModal()">Cancel</button>
            <button class="btn-modal btn-submit" onclick="hsaSubmitAssign()">✅ Assign</button>
        </div>
    </div>
</div>
...
function toggleActions(btn, event) {
    ...
    const cell = btn.closest('.saas-actions-cell');
...
function hsaOpenAssignModal(bookingId, patientName) {
...
function hsaCloseAssignModal() {
    document.getElementById('assignModal').classList.remove('show');
    document.body.style.overflow = 'auto';
    currentAssignBookingId = null;
}

function hsaSubmitAssign() {
    ...
    fetch(`/hospital-super-admin/bookings/${currentAssignBookingId}/assign`, {
        method: 'POST',
        credentials: 'include',
        headers: {
...
window.addEventListener('click', function(e) {
    ...
    if (e.target === m) hsaCloseAssignModal();
});

// Update booking status
function hsaUpdateStatus(id, status) {
    ...
    fetch(`/hospital-super-admin/bookings/${id}/status`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
function hsaOpenRescheduleModal(bookingId, patientName, bookingDate, startTime) {
...
function hsaCloseRescheduleModal() {
    document.getElementById('rescheduleModal').classList.remove('show');
    document.body.style.overflow = 'auto';
}

// Submit reschedule
function hsaSubmitReschedule() {
    ...
    fetch(`/hospital-super-admin/bookings/${bookingId}/reschedule`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
